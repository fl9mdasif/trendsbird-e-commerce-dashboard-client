...
function ToastProvider({ timeout = 2500, ...props }: ToastPrimitive.Provider.Props & { timeout?: number }) {
...
function ToastIcon({ type }: { type: string | undefined }) {
  ...
  if (type === "loading") {
    icon = <Loader2Icon className="animate-spin text-primary" aria-hidden="true" />;
  }
...
function Toaster({
  ...
  timeout = 2500,
  ...props
}: ToastPrimitive.Provider.Props & { timeout?: number }) {
  return (
...
export const toast = (options: ToastOptions) => {
  const toastType = options.type || (options.variant === "destructive" ? "error" : "info");
